...
3. **進行中の作業を TaskCreate / TaskUpdate で追跡する**: 今着手しているタスクだけを登録し、進捗を更新する。全タスクを一括登録する必要はない。
4. **todoを見ながら実装する**: 完了したtodoにチェックを付けて進捗を可視化する。
5. **問題が起きたらリプランする**: 無理に突き進まない。立ち止まってプランモードに戻り、todoを更新してから方針を見直す。

**大規模タスク**（設計判断を伴う・複数コンポーネントにまたがる・アーキテクチャ変更）:
上記に加え、以下を行う。
- **設計書を書く**: 何を作るか（仕様）・どう作るか（設計）を1つの文書にまとめる。全体像・方針・判断根拠を明文化し、作業の拠り所にする。保存先はプロジェクトの規約に従う（規約がなければ `.claude/design/`）。
- 実装中は常に設計書を参照し、方向を見失わない。乖離があれば設計に立ち返る。
...
- **方針との突き合わせ**: 実装がtodoファイルの設計メモ（中規模）または設計書（大規模）と合致しているか確認する。乖離があれば立ち返る。
